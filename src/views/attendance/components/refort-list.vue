<template>
  <div class="boxInfo">
    ...
    >
      ...
      <el-table-column prop="hoursPerWorkDay" label="日均时长(工作日)" width="150" />
      <el-table-column prop="hoursPerRestDay" label="日均时长(休息日)" width="150" />
      <el-table-column prop="absenceDays" label="旷工天数" width="100" />
      <el-table-column prop="isFullAttendanceint" label="是否全勤" width="100" />
      <el-table-column prop="actualAtteOfficialDays" label="实际出勤天数" width="180" />
      <el-table-column prop="actualAtteUnofficialDays" label="实际出勤天数(非正式)" width="180" />
      <el-table-column prop="workingDays" label="应出勤工作日" width="120" />
      <el-table-column prop="salaryStandards" label="计薪标准" width="100" />
      <el-table-column prop="salaryOfficialDays" label="计薪天数" width="150" />
      <el-table-column prop="salaryUnofficialDays" label="计薪天数(非正式)" width="150" />
    </el-table>
    <!-- 分页 -->
...
<script>
import { archives, exportHistoricalExcel, newReports, reportFormList } from '@/api/attendance'
...
export default {
  ...
  methods: {
    ...
    async  reportFormList(params) {
      this.loading = true
      this.dataList = await reportFormList({ atteDate: this.month })
      this.loading = false
    },
    // 新增用户刷新列表
    handleLoadDataList() {
    ...
    async handelFileDownload() {
      try {
        const res = await exportHistoricalExcel(this.month, 1)
        FileSaver.saveAs(res, `${this.month}员工出勤报表.xlsx`)
      } catch (error) {
